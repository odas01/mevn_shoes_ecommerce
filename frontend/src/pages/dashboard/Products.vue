...
export default {
  ...
  data() {
    const contactFormSchema = yup.object().shape({
      ...
      size: yup
        .string()
        .required("Size is required. Multiple sizes separated by space"),
      stock: yup
        .number()
...
<template>
  ...
  <div class="db__content-header">
    <input
      ...
      @keyup.enter="seacrhProducts"
    />
    <select name="type" id="" @change="setFilter($event, 'category')">
      <option value="">--- Category ---</option>
      <option v-for="cate in categorys" :value="cate.name">
        {{ cate.name }}
      </option>
    </select>
  ...
  <Drawer
    :title="!idEdit ? 'ADD PRODUCT' : 'edit product'"
    placement="right"
    :closable="true"
    :visible="visible"
    @close="closeDrawer"
  >
    ...
    >
      ...
      <div>
        ...
        >
          <option v-for="category in converCategory" :value="category.name">
            {{ category.name }}
          </option>
        </Field>
      ...
      <div>
        ...
        <Field as="select" name="category" id="" v-model="productEdit.category">
          <option v-for="category in categoryActive.children" :value="category">
            {{ category }}
          </option>
        </Field>
  ...
  <div class="db__body" v-if="products.length > 0">
      ...
      <tbody>
        <tr v-for="(product, index) in products">
          ...
          <td style="text-transform: capitalize">
            <img :src="menImage" v-if="product.type === 'men'" alt="men" />
            <img
              :src="womenImage"
              v-if="product.type === 'women'"
              alt="women"
            />
            <img :src="kidImage" v-if="product.type === 'kid'" alt="kid" />
          </td>
          <td>
          ...
          <td class="text-center">
            <span class="product__status" v-if="product.stock > 0"
              >Selling</span
            >
            <span class="product__status product__status-soldout" v-else
              >Sold Out</span
            >
          </td>
          <td class="text-center">
          ...
          <td>
            <div class="product__actions">
              <i
                ...
                @click="toDetail(product._id)"
              ></i>
              <i
                class="fa-solid fa-pen-to-square"
                @click="showDrawerEdit(product)"
              >
              </i>
              <i
    ...
    <div class="pagi" v-if="!isSearch && !isFilter">
      ...
      <ul class="pagi-list">
        ...
        <li
          class="pagi-item"
          v-for="(index, item) in paginationCount"
          :class="{ active: pageCur + 1 === index }"
          @click="changePagination(index)"
        >
        ...
        <li class="pagi-item">
          <button
            class="pagi-btn"
            :class="{ disabled: pageCur === paginationCount - 1 }"
            @click="pageCur++"
          >
            Next
          </button>
